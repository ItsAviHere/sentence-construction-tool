  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4 py-6">
      <div className="bg-white rounded-2xl shadow-lg w-full max-w-2xl p-6 sm:p-8">
        ...
        <button className="text-sm text-gray-500 hover:text-red-500 float-end bg-gray-200 p-1 w-12 mr-3 rounded-lg">
          Quit
        </button>
          ...
          {current.sentence.split(/(\{\d+\})/g).map((part, i) => {
            ...
            if (match) {
              ...
              return (
                <span
                  key={i}
                  className="inline-block border px-2 py-1 mx-1 cursor-pointer bg-gray-100"
          ...
          {current.options.map((word, i) => (
            <button
              ...
              className={`px-3 py-1 border rounded ${
                selected.includes(word) ? "bg-gray-300" : "bg-blue-100"
              }`}
        ...
        <div className="flex justify-end">
          <button
            className=" bg-sky-700 text-white px-4 py-2 rounded disabled:opacity-50"
